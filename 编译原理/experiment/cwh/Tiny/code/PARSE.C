...
static TreeNode * varlist(ExpType expType);
...
static TreeNode * decl(void)
{
	...
	{
	case INT:
		p->child[0]=newDeclNode(DeclId);
		p->child[0]->attr.name = copyString(tokenString);
		match(token);
		p->child[0]->sibling=varlist(Integer);
		match(SEMI);
		break;
	case BOOL:
		p->child[0]=newDeclNode(DeclId);
		p->child[0]->attr.name = copyString(tokenString);
		match(token);
		p->child[0]->sibling=varlist(Boolean);
		match(SEMI);
		break;
	case CHAR:
		p->child[0]=newDeclNode(DeclId);
		p->child[0]->attr.name = copyString(tokenString);
		match(token);
		p->child[0]->sibling=varlist(Char);
		match(SEMI);
		break;
	default:
		p=stmt_sequence();
...
static TreeNode * varlist(ExpType expType)
{
	TreeNode * t = newDeclNode(expType);
	TreeNode * p = t;//use to point to sibling
	if ((t!=NULL) && (token==ID))
	{
			t->attr.name = copyString(tokenString);
			t->kind.expType=expType;
	}
	match(ID);
	while (token!=SEMI)
	{ TreeNode * q;
